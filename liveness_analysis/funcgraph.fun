...
struct
    ...
    struct
        ...
        fun compare({from=f1, to=t1},{from=f2, to=t2}) =
            case Key.compare(f1, f2)
            of	EQUAL => Key.compare(t1, t2)
            | 	x => x
    end
    ...
    fun addNewNode (g, nid, d) =
        case NodeMap.find (g, nid)
        of 	NONE => addNode (g, nid, d)
        | 	SOME x => g
    fun changeNodeData (g, nid, d) =
        let val (_, _, s, p) = getNode (g, nid)
        in NodeMap.insert(g, nid, (nid, d, s, p))
        end

    fun adjustSets (g, {from, to}, f) =
        case Key.compare (from, to)
        of	EQUAL =>
            let val (i, d, s, p) = getNode (g, from)
            in NodeMap.insert (g, from, (i, d, f (s, from), f (p, from)))
            end
        | 	_ =>
            let val (fi, fd, fs, fp) = getNode (g, from)
                val (ti, td, ts, tp) = getNode (g, to)
                val fs' = f (fs, to)
                val tp' = f (tp, from)
            in
                NodeMap.insert (
                    NodeMap.insert(g, from, (fi, fd, fs', fp)),
                    to, (ti, td, ts, tp')
                )
            end
    ...
    fun doubleEdge (g, temp1, temp2) =
        case Key.compare (temp1, temp2)
            of 	EQUAL => g
            |  	_ =>
                let val g' = addEdge (g, {from=temp1, to=temp2})
                in addEdge (g', {from=temp2, to=temp1})
                end
    ...
    fun removeNode (g, nid) =
        let val (_, _, succ, pred) = getNode (g, nid)
            val es1 =
                NodeSet.foldl
                (fn (s, es) => EdgeSet.add (es, {from=nid, to=s}))
                EdgeSet.empty
                succ
            val es2 =
                NodeSet.foldl
                (fn (p, es) => EdgeSet.add (es, {from=p, to=nid}))
                es1
                pred
    ...
    fun writeGraph outstream stringify undir g =
        let fun writeln x = TextIO.output (outstream, x ^ "\n")
            ...
            fun writeSet s = NodeSet.app (writeln o stringNid) s
            fun writeNode (nid, data, succs, preds) =
                let val s = "Node: " ^ stringify (nid,data)
                in
                    if undir
                    then (
                        writeln (s ^ " -> Neighbors:");
        ...
        in NodeMap.app writeNode g
        end

    fun printGraph stringify undir g = writeGraph TextIO.stdOut stringify undir g
