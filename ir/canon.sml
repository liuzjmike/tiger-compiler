  ...
  fun linearize(stm0: T.stm) : T.stm list =
      let
        ...
        fun reorder ((e as T.CALL _ )::rest) =
            ...
            in reorder(T.ESEQ(T.MOVE(T.TEMP t, e), T.LVALUE(T.TEMP t)) :: rest)
            end
          | reorder (a::rest) =
            let
              val (stms,e) = do_exp a
            ...
            in
              ...
              then (stms % stms',e::el)
                ...
                let val t = Temp.newtemp()
                in (stms % T.MOVE(T.TEMP t, e) % stms', T.LVALUE(T.TEMP t) :: el)
                end
            end
          | reorder nil = (nop,nil)
        ...
        and do_stm(T.SEQ(a,b)) = 
          ...
          | do_stm(T.CJUMP(e,t,f)) = 
            reorder_stm([e], fn[e]=> T.CJUMP(e,t,f))
        ...
        and do_exp(T.BINOP(p,a,b)) = 
          ...
          | do_exp(T.RELOP(p,a,b)) =
            reorder_exp([a,b], fn[a,b]=>T.RELOP(p,a,b))
          | do_exp(T.LVALUE(T.MEM(a))) = 
            reorder_exp([a], fn[a]=>T.LVALUE(T.MEM(a)))
          | do_exp(T.ESEQ(s,e)) = 
            let
              val stms = do_stm s
              val (stms',e) = do_exp e
      ...
      | (most,T.CJUMP(e,t,f)) =>
          (case (Symbol.look(table,t), Symbol.look(table,f))
            of (_, SOME(b' as _::_)) => b @ trace(table, b', rest)
             | (SOME(b' as _::_), _) => 
		           most @ [T.CJUMP(T.notRel e,f,t)]
		                @ trace(table, b', rest)
             | _ => let val f' = Temp.newlabel()
		     in most @ [T.CJUMP(e,t,f'), 
